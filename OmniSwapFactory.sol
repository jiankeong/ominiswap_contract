...
contract SwapPair is ISwapPair, SwapERC20 {
    ...
    event BurnToken(address token, uint amount);
    event DistributeToken(address token, address[] feeAddressList, uint[] feePercentList);
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external override lock {
        ...
        emit SwapLog(tx.origin, amount0In, amount1In, amount0Out, amount1Out, block.timestamp);
    }
    ...
    function distributeToken(address token, address[] memory feeAddressList, uint[] memory feeList) external override lock {
        ...
        uint balance1 = IERC20(token1).balanceOf(address(this));
        _update(balance0, balance1, _reserve0, _reserve1);
        emit DistributeToken(token, feeAddressList, feeList);
    }
...
contract OmniSwapFactory is ISwapFactory, Ownable {
    bytes32 public override constant initCodeHash = keccak256(abi.encodePacked(type(SwapPair).creationCode));
    uint256 public constant FEE_RATE_BASE = 10000;

    address public override feeTo;
    address public override router;
    uint256 public override swapFee = 200;
    // uint256 public protocolFee = 10000;
