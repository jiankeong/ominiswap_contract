...
contract OMINTv2 is AdminRole {
    ...
    mapping(uint256 => uint256) public priceOmni;
    ...
    constructor(address onft_,address relation_){
      ...
      priceOmni[1] = 50* 10** 18;
      priceOmni[2] = 250* 10** 18;
      priceOmni[3] = 500* 10** 18;
      priceOmni[4] = 2500* 10** 18;
      priceOmni[5] = 5000* 10** 18;
      priceOmni[6] = 25000* 10** 18;
      priceOmni[7] = 50000* 10** 18;
  ...
  function Mint(uint256 typeID) external callerIsUser {
    ...
    function MintOmni(uint256 typeID) external callerIsUser {
    require(
      INFT(onft).totalSupplyOfType(typeID) < amountForType[typeID],
      "Insufficient reserve to support desired mint amount"
    );
    ...
    uint256 OMNIAmount = price[typeID];
    IERC20(OMNI).transferFrom(msg.sender,fundAddress,OMNIAmount/1000000); 
    INFT(onft).mint(msg.sender,typeID,1);
    mintStats[msg.sender] = true;
    _record(msg.sender,OMNIAmount);
    }
